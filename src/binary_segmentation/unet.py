...
def conv2d(x, W,keep_prob_=1.0, pad=True):
...
def deconv2d(x, W,stride, pad=False, factor=2):
    ...
    channels = tf.cast(tf.round(tf.cast(x_shape[3], tf.float32)//factor), tf.int32)
    output_shape = tf.stack([x_shape[0], x_shape[1]*stride, x_shape[2]*stride, channels ])
...
def model(x, channels, n_class, layers=3, features_root=64, filter_size=3, pool_size=2, summaries=True, two_sublayers=True, training=True, keep_prob=1.0):
    ...
    for layer in range(0, layers):
        with tf.variable_scope('down'+str(layer)) as scope:
            features = np.round(ch_increase_factor**layer*features_root)
            ...
            if(FLAGS.unet_shortskip):
                if layer == 0:
                    ...
                    dw_h_convs[layer] = res_block(features, features//ch_increase_factor, in_node, filter_size, train=training, keep_prob=keep_prob)
                ...
                if layer == 0:
                    ...
                    print(features//ch_increase_factor)
                    w1 = weight_variable("w1", [filter_size, filter_size, features//ch_increase_factor, features], stddev)
    ...
    for layer in range(layers-2, -1, -1):
        with tf.variable_scope('up'+str(layer)) as scope:
            features = np.round(ch_increase_factor**(layer+1)*features_root)
            stddev = tf.sqrt(2 / (filter_size**2 * features))
            print(features)
            wd = weight_variable_deconv("wd",[pool_size, pool_size, features//ch_increase_factor, features], stddev)
            print(in_node)
            print(wd)
            bd = bias_variable("bd",[features//ch_increase_factor])
            print(bd)
            h_deconv = deconv2d(in_node, wd, pool_size, factor=ch_increase_factor) + bd
            print(h_deconv)
            ...
            if(FLAGS.unet_shortskip):
                ...
                w1 = weight_variable("w1", [filter_size, filter_size, features, features//ch_increase_factor], stddev)
                b1 = bias_variable("b1", [features//ch_increase_factor])
                ...
                if(FLAGS.unet_dualconv):
                    w2 = weight_variable("w2", [filter_size, filter_size, features//ch_increase_factor, features//ch_increase_factor], stddev)
                    b2 = bias_variable("b2",[features//ch_increase_factor])
