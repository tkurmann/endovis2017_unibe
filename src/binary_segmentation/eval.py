...
def plot_results(images_value, labels_value, logits_value, crf_value,  step):
        ...
        plt.subplot(141)
        plt.imshow(images_value[0,:,:,:], vmin=0, vmax=1.0)
        plt.axis('off')
        ...
        plt.subplot(142)
        plt.imshow(logits_value[0,:,:,0], vmin=0, vmax=1.0)
        plt.axis('off')
        # plt.imshow(mask, cmap='jet', alpha=0.5)

        plt.subplot(143)
        plt.imshow(crf_value[0,:,:], vmin=0, vmax=2)
        plt.axis('off')

        plt.subplot(144)
        plt.imshow(labels_value[0,:,:,0] > 0.5)
        plt.axis('off')
    ...
    if int(global_step) > int(old_step):
        ...
        try:
            ...
            while step < num_iter and not coord.should_stop():
                ...
                # images_s = exposure.equalize_adapthist(images_s[0,:,:,:])
                # images_s = np.expand_dims(images_s, axis=0)

                images_value = images_s
                # mean = np.mean(images_s)
                # adjusted_stddev = max(np.std(images_s), 1.0/np.sqrt(images_s.size))
                # images_s = (images_s-mean)/adjusted_stddev
                ...
                for r in range(4):
                    ...
                    # logits_value_sigmoid = logits_value_sigmoid + softmax(logits_value_r)
                ...
                # np.save("results", np.concatenate((images_value, logits_value),axis=3))
                ...
                plot_results(images_value, labels_value, logits_value, predictions_value, step)
...
def evaluate():
  with tf.Graph().as_default() as g:
    ...
    crf_logits = logits_placeholder
    predictions = tf.py_func(dense_crf, [crf_logits, tf.cast(images_placeholder*255,tf.uint8)], tf.float32)
