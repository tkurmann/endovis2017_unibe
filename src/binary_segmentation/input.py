...
def read_and_decode(filename_queue):
   ...
   # [mnist.IMAGE_PIXELS].
    ...
    label_shape = tf.stack([height, width, 2])
    ...
    # label_probe = tf.expand_dims(label_probe,2)
    # # label = tf.concat((label_corr_0, label_corr_1, label_probe), axis=2)
    # label = tf.concat((label_corr_0, label_corr_1), axis=2)

    # label_corr_0 = 1 - label_probe
    # label_corr_0 = tf.expand_dims(label_corr_0,2)
    # label_corr_1 = label_probe
    # label_corr_1 = tf.expand_dims(label_corr_1,2)
    # label = tf.concat((label_corr_0, label_corr_1), axis=2)
...
def distorted_inputs_files(filename, batch_size):
  ...
  with tf.device('/cpu:0'):
        ...
        if(FLAGS.data_augmentation_level == 9):
            ...
            image, label = random_horizontal_flip(image, label)
            image, label = random_vertical_flip(image, label)
            image, label = random_shift(image, label, max_shift=200)
            image, label = random_rotate(image, label, max_angle=45)
