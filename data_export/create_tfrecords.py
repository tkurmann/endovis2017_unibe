...
def convert_to(left_images, right_images,  labels, name):
  """Converts a dataset to tfrecords."""
  # images = data_set.images
  # labels = data_set.labels
  num_examples = left_images.shape[0]

  if left_images.shape[0] != num_examples:
    raise ValueError('Images size %d does not match label size %d.' %
                     (images.shape[0], num_examples))
  rows = left_images.shape[1]
  cols = left_images.shape[2]
  try:
      depth = left_images.shape[3]
  except:
      depth = 1

  filename = os.path.join("tf2/", name + '.tfrecords')
  ...
  for index in range(num_examples):
    left_image_raw = left_images[index].tostring()
    right_image_raw = right_images[index].tostring()
    ...
    def generate_binary_labels(self, instrument_parts, instrument_types):
        ...
        # for f in range(1):
            ...
            for g in range(len(fileset.ground_truth_files)):
                ...
                # if(instrument_type_id == 6): # If its other it isnt an instrument....
...
def main(unused_argv):
    ...
    with open(DATA_BASE_DIR+instrument_parts_file) as data_file:
        instrument_parts = json.load(data_file)

    with open(DATA_BASE_DIR+instrument_type_file) as data_file:
        instrument_types = json.load(data_file)

    instrument_list = []

    instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_1/left_frames",DATA_BASE_DIR+"instrument_dataset_1/right_frames",[DATA_BASE_DIR+"instrument_dataset_1/ground_truth/Maryland_Bipolar_Forceps_labels",DATA_BASE_DIR+"instrument_dataset_1/ground_truth/Other_labels",DATA_BASE_DIR+"instrument_dataset_1/ground_truth/Prograsp_Forceps_labels" ],["Bipolar Forceps","Other","Prograsp Forceps" ]))

    instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_2/left_frames",DATA_BASE_DIR+"instrument_dataset_2/right_frames", [DATA_BASE_DIR+"instrument_dataset_2/ground_truth/Left_Prograsp_Forceps_labels",DATA_BASE_DIR+"instrument_dataset_2/ground_truth/Other_labels",DATA_BASE_DIR+"instrument_dataset_2/ground_truth/Right_Prograsp_Forceps_labels" ],["Prograsp Forceps","Other","Prograsp Forceps" ]))

    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_3/left_frames",DATA_BASE_DIR+"instrument_dataset_3/right_frames",[DATA_BASE_DIR+"instrument_dataset_3/ground_truth/Left_Large_Needle_Driver_labels",DATA_BASE_DIR+"instrument_dataset_3/ground_truth/Right_Large_Needle_Driver_labels" ],["Large Needle Driver","Large Needle Driver"]))
    # #
    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_4/left_frames",DATA_BASE_DIR+"instrument_dataset_4/right_frames",[DATA_BASE_DIR+"instrument_dataset_4/ground_truth/Large_Needle_Driver_Left_labels",DATA_BASE_DIR+"instrument_dataset_4/ground_truth/Large_Needle_Driver_Right_labels",DATA_BASE_DIR+"instrument_dataset_4/ground_truth/Prograsp_Forceps_labels" ],["Large Needle Driver","Large Needle Driver","Prograsp Forceps" ]))
    #
    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_5/left_frames",DATA_BASE_DIR+"instrument_dataset_5/right_frames",[DATA_BASE_DIR+"instrument_dataset_5/ground_truth/Bipolar_Forceps_labels",DATA_BASE_DIR+"instrument_dataset_5/ground_truth/Grasping_Retractor_labels",DATA_BASE_DIR+"instrument_dataset_5/ground_truth/Vessel_Sealer_labels" ],["Bipolar Forceps","Grasping Retractor","Vessel Sealer" ]))
    #
    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_6/left_frames",DATA_BASE_DIR+"instrument_dataset_6/right_frames",
    # [DATA_BASE_DIR+"instrument_dataset_6/ground_truth/Left_Large_Needle_Driver_labels",
    # DATA_BASE_DIR+"instrument_dataset_6/ground_truth/Monopolar_Curved_Scissors_labels",
    # DATA_BASE_DIR+"instrument_dataset_6/ground_truth/Prograsp_Forceps", DATA_BASE_DIR+"instrument_dataset_6/ground_truth/Right_Large_Needle_Driver_labels" ]
    # ,["Large Needle Driver","Monopolar Curved Scissors","Prograsp Forceps", "Large Needle Driver" ]))
    #
    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_7/left_frames",DATA_BASE_DIR+"instrument_dataset_7/right_frames", [DATA_BASE_DIR+"instrument_dataset_7/ground_truth/Left_Bipolar_Forceps",DATA_BASE_DIR+"instrument_dataset_7/ground_truth/Right_Vessel_Sealer" ],["Bipolar Forceps","Vessel Sealer"]))
    #
    #
    # instrument_list.append(InstrumentSet(DATA_BASE_DIR+"instrument_dataset_8/left_frames",DATA_BASE_DIR+"instrument_dataset_8/right_frames", [DATA_BASE_DIR+"instrument_dataset_8/ground_truth/Bipolar_Forceps_labels",DATA_BASE_DIR+"instrument_dataset_8/ground_truth/Left_Grasping_Retractor_labels",DATA_BASE_DIR+"instrument_dataset_8/ground_truth/Monopolar_Curved_Scissors_labels", DATA_BASE_DIR+"instrument_dataset_8/ground_truth/Right_Grasping_Retractor_labels" ],["Bipolar Forceps","Grasping Retractor", "Monopolar Curved Scissors", "Grasping Retractor" ]))


    instrument_idx = list(range(len(instrument_list)))

    for leave_out in instrument_idx:
        print(leave_out)
        leave_out = leave_out
        eval_set = []
        eval_set.append(instrument_list[leave_out])
        eval_set = np.asarray(eval_set)
        training_set = np.array(instrument_list[:leave_out]+instrument_list[leave_out+1:])

        left_images = np.empty([0,IMAGE_HEIGHT,IMAGE_WIDTH, 3]).astype(np.uint8)
        right_images = np.empty([0,IMAGE_HEIGHT,IMAGE_WIDTH, 3]).astype(np.uint8)
        labels = np.empty([0,IMAGE_HEIGHT,IMAGE_WIDTH, 15]).astype(np.uint8)

        for instrument in training_set:
            instrument.gather_data_files()
            instrument.gather_and_map_ground_truths()
            left_image_data, right_image_data, gt_data =  instrument.generate_binary_labels(instrument_parts, instrument_types)

            left_images = np.vstack((left_images, left_image_data))
            right_images = np.vstack((right_images, right_image_data))
            labels = np.vstack((labels, gt_data))
            # print(np.std(right_images, axis=(0,1,2)))
            # print(np.mean(right_images, axis=(0,1,2)))




        convert_to(left_images, right_images , labels, "binary_train_"+str(leave_out)+"_probe")
        ...
        convert_to(left_images, right_images , labels, "binary_eval_"+str(leave_out)+"_probe")


    # for instrument in eval_set:
    #
    # instrument1.gather_data_files()
    # instrument1.gather_and_map_ground_truths()
    # instrument1.generate_parts_instruments_labels(instrument_parts, instrument_types)
    # sets = instrument1.filesets
    #
    #
    # print(instrument_parts)
