...
def convert_to(images,  labels, binary_segmentation,  name):
  ...
  for index in range(num_examples):
    ...
    binary_segmentation_raw = binary_segmentation[index].tostring()
    example = tf.train.Example(features=tf.train.Features(feature={
        'height': _int64_feature(rows),
        'width': _int64_feature(cols),
        'depth': _int64_feature(depth),
        'label': _bytes_feature(label_raw),
        'binary_segmentation': _bytes_feature(binary_segmentation_raw),
...
class InstrumentSet():
    ...
    def generate_binary_labels(self, instrument_parts, instrument_types):
        ...
        # print(gt_data.shape)
        return left_image_data,  instrument_gt_data
...
def main(unused_argv):
    ...
    binary_segmentations = np.empty([0,IMAGE_HEIGHT,IMAGE_WIDTH]).astype(np.uint8)
    labels = np.empty([0,IMAGE_HEIGHT,IMAGE_WIDTH, 7]).astype(np.uint8)
    ...
    for instrument in set:
        ...
        binary_result_data = np.zeros([len(left_image_data),IMAGE_HEIGHT,IMAGE_WIDTH]).astype(np.uint8)

        for b in range(len(left_image_data)):
            binary_result = crop_frame(misc.imread(FLAGS.binary_results_folder + "frame_logits_"+str(bs_file)+".png"))
            binary_result_data[b,:,:] = binary_result
            bs_file+=1

        binary_segmentations = np.vstack((binary_segmentations, binary_result_data))


        print(binary_segmentations.shape)
        print(left_images.shape)
        print(labels.shape)

    convert_to(left_images, labels,binary_segmentations, "type_"+FLAGS.split+"_"+str(leave_out))
